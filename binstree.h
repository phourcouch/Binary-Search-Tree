...
class BST
{
 ...
 public:
  ...
  BST();  // intializes Root
  ~BST();  // destructor calls dtraverse to destroy the dynamic tre
 
//PURPOSE: call height() and BF() to find vertex's height and BF and then print it out
//PARAMTER is the vertex you want to find the height and BF of 
 void climbUp(Vertex*);

//PURPOSE: call correct rotation rountines based on V's BF and the BF of a sub tree
//PARETER: Vertex is the bad vertex 
  void fixIt(Vertex*);

//PURPOSE: calcute the longest path of a vertex (the height of it)
//PARAMTER is the vertex you want to find the height of 
   int height(Vertex* );

//PURPOSE: calculate Balance Facotr of a Vertex
//PARAMETER: Vertex you want to find the BF of 
   int BF(Vertex* );

//PURPSOE: do a right rotation  - tree is  left  heavy and pass back pivot
//PARA: Vertex being passed is the bad one (BF is 2 or -2)
  Vertex* rotateRight(Vertex*);

//PURPOSE: do  left rotation - tree is right heavy and pass  back pivot
//PARA: Bad Vertex - has BF of 2 or -2
